...
const checkIfGoodUser = (req, res, next) => {
  ...
  if (email === "malika.ink84@gmail.com" && password === "FoireAuxPlantesCP4") {
    ...
    res
      .status(403)
      .send(`Désolé ! Vous n'êtes pas autorisé.e à accéder à cette route...`);
  }
};
...
const userSchema = Joi.object({
  email: Joi.string()
    .email()
    .messages({ "string.email": `Merci de renseigner un email valide.` })
    .required(),
  pseudo: Joi.string()
    .messages({ "string.empty": `Merci de renseigner un pseudo valide.` })
    .required(),
